...
class TestInstallNode:
    def test_downloads_the_configured_version(
        self, fake_run: FakeRun, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        # The download is faked, so there is no tarball to extract.
        monkeypatch.setattr(pkgs.tarfile, "open", lambda *a, **k: _NullTar())
        pkgs._install_node(tmp_path, "x64", {"version": "22.14.0"})
        assert any("22.14.0" in ln for ln in fake_run.command_lines)
